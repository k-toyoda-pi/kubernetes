...
func testRecreateDeployment(f *framework.Framework) {
	...
	err = framework.WatchRecreateDeployment(c, deployment)
	framework.ExpectNoError(err)
}
...
func testIterativeDeployments(f *framework.Framework) {
	...
	err = framework.WaitForDeploymentWithCondition(c, ns, deploymentName, deploymentutil.NewRSAvailableReason, apps.DeploymentProgressing)
	framework.ExpectNoError(err)
}
...
func testProportionalScalingDeployment(f *framework.Framework) {
	...
	if deployment.Status.AvailableReplicas < minAvailableReplicas {
		framework.ExpectNoError(fmt.Errorf("observed %d available replicas, less than min required %d", deployment.Status.AvailableReplicas, minAvailableReplicas))
	}
